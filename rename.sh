...
fvm.bat flutter pub run change_app_package_name:main $bundleId
...
grep --exclude=./rename.sh -r -l "fhrstic_task" . | sort | uniq | xargs perl -e "s/fhrstic_task/$packageName/" -pi

grep --exclude=./rename.sh -r -l "fhrstic_task" . | sort | uniq | xargs perl -e "s/fhrstic_task/$appName/" -pi

grep --exclude=./rename.sh -r -l "com.fhrstic.task.dev" . | sort | uniq | xargs perl -e "s/com.fhrstic.task.dev/$bundleId.dev/" -pi

grep --exclude=./rename.sh -r -l "com.fhrstic.task" . | sort | uniq | xargs perl -e "s/com.fhrstic.task/$bundleId/" -pi
...
find . -depth -name "fhrstic_task.iml" -exec sh -c 'f="{}"; mv -- "$f" "$packageName.iml"' \;
find . -depth -name "fhrstic_task_android.iml" -exec sh -c 'f="{}"; mv -- "$f" "$packageName_android.iml"' \;
